 ...
 */
class XUploadAction extends \yii\base\Action
{
    /**
     ...
     * @since 0.5
     */
    public $formClass = 'makroxyz\models\UploadForm';
    /**
     ...
     * @since 0.5
     */
    public $mimeTypeAttribute = 'mimeType';
    /**
...
//    public $stateVariable = 'xuploadFiles';
    ...
    public function init()
    {
        ...
        if ($this->thumbPath === null) {
            $this->thumbPath = $this->uploadDest . DIRECTORY_SEPARATOR . 'thumb';
        } else {
        ...
        if ($this->createDirs && !is_dir($this->thumbPath)) {
...
//        }
        if (!isset($this->_formModel)) {
            $this->formModel = Yii::createComponent($this->formClass);
        }
    ...
    {
        $this->sendHeaders();
        $this->handleDeleting() || $this->handleUploading();
    }
                ...
                if ($returnValue === true) {
                    return Json::encode([
                        'files' => [
                            ...
                            "deleteUrl" => $this->getController()->createUrl($this->getId(), [
                                "_method" => "delete",
                    ...
                    Yii::error("XUploadAction: " . $returnValue, "xupload.actions.XUploadAction");
                }
            } else {
...
//        Yii::app()->user->setState($this->stateVariable, $userFiles);
    ...
    {
        $path = ($this->_subfolder != "") ? "{$this->uploadPath}/{$this->_subfolder}/" : "{$this->uploadPath}/";
        return $path;
    }
...
//    }
    ...
    protected function afterValidateError($model)
    {
        ...
        Yii::error("XUploadAction: " . CVarDumper::dumpAsString($model->getErrors()), "xupload.actions.XUploadAction");
    }
}